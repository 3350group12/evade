		...
		if (e->xbutton.button==1) {
			...
			if (ts > 0.1) {
				...
				if (g.nbullets < MAX_BULLETS) {
					...
					b->vel[1] = 20;
                    //second bullet
					b2->pos[0] = g.ship2.pos[0];
					b2->pos[1] = g.ship2.pos[1];
					b2->vel[0] = g.ship2.vel[0];
					b2->vel[1] = g.ship2.vel[1];
					b2->vel[1] = 20;
...
//					b->vel[1] += ydir*SHIFT_VEL + rnd()*0.1;
					...
					b->super = false;
					b2->color[0] = BULLET_COLOR;
					b2->color[1] = BULLET_COLOR;
					b2->color[2] = BULLET_COLOR;
					b2->super = false;
					++g.nbullets;
				}
			}
...
//			a->vel[1] = (Flt)(rnd()*2.0-1.0);
	...
	Flt d0,d1,dist, d02, d12, dist2;
	//Update ship position
	g.ship.pos[0] += g.ship.vel[0];
	g.ship.pos[1] += g.ship.vel[1];
    g.ship2.pos[0] = g.ship.pos[0] + 40;
    g.ship2.pos[1] = g.ship.pos[1];
	...
	while (i < g.nbullets) {
		...
		if (ts > 2.5) {
			...
			memcpy(&g.barr[i], &g.barr[g.nbullets-1],
				sizeof(Bullet));
			g.nbullets--;
			memcpy(&g.barr2[i], &g.barr2[g.nbullets-1],
				sizeof(Bullet));
			g.nbullets--;
			//do not increment i.
			continue;
...
//		}
	...
	while (a) {
		...
		while (i < g.nbullets) {
			...
			d1 = b->pos[1] - a->pos[1];
			dist = (d0*d0 + d1*d1);
			d02 = b2->pos[0] - a->pos[0];
			d12 = b2->pos[1] - a->pos[1];
			dist2 = (d02*d02 + d12*d12);
			if ((dist < (a->radius*a->radius) || dist2 < (a->radius*a->radius)) && !b->super) {
...
void game_render()
{
	...
	for (int i=0; i<g.nbullets; i++) {
		...
		glColor3f(1.0, 1.0, 1.0);
		glBegin(GL_POINTS);
			glVertex2f(b2->pos[0],      b2->pos[1]);
			glVertex2f(b2->pos[0]-1.0f, b2->pos[1]);
			glVertex2f(b2->pos[0]+1.0f, b2->pos[1]);
			glVertex2f(b2->pos[0],      b2->pos[1]-1.0f);
			glVertex2f(b2->pos[0],      b2->pos[1]+1.0f);
			glColor3f(0.8, 0.8, 0.8);
			glVertex2f(b2->pos[0]-1.0f, b2->pos[1]-1.0f);
			glVertex2f(b2->pos[0]-1.0f, b2->pos[1]+1.0f);
			glVertex2f(b2->pos[0]+1.0f, b2->pos[1]-1.0f);
			glVertex2f(b2->pos[0]+1.0f, b2->pos[1]+1.0f);
		glEnd();
